...
class Affair(db.Model):
    ...
    affair_type = db.Column(db.Integer, db.ForeignKey('cyc_affair_type.affair_type'))
    a_type = db.relationship('Affair_type', backref='a_type')

    affair_commit_time = db.Column(db.TIMESTAMP, server_default=text('CURRENT_TIMESTAMP'),
                                   server_onupdate=text('CURRENT_TIMESTAMP'))
    affair_finish_time = db.Column(db.DateTime)
    affair_num = db.Column(db.Integer)
    part_id = db.Column(db.Integer)
    order_id = db.Column(db.Integer)
    # part_name = db.Column(db.String(16))
    affair_status = db.Column(db.Integer, db.ForeignKey('cyc_affair_status.status_type'))
    a_status = db.relationship('Affair_status', backref='a_status')
...
class Order(db.Model):
    ...
    # 还须采购的零件数量
    purchased_num = db.Column(db.Integer)
    ...
    order_status = db.Column(db.Integer, db.ForeignKey('cyc_order_status.status_type'))
    o_status = db.relationship('Order_status', backref='o_status')
...
class Order_status(db.Model):
    # 订货信息状态
    # 定义表名
    __tablename__ = 'cyc_order_status'
    status_type = db.Column(db.Integer, primary_key=True)
    status_explain = db.Column(db.String(7))

    s_order = db.relationship(Order, backref='s_order')


class Affair_type(db.Model):
    # 事务信息类型
    # 定义表名
    __tablename__ = 'cyc_affair_type'
    affair_type = db.Column(db.Integer, primary_key=True)
    type_explain = db.Column(db.String(2))


class Affair_status(db.Model):
    # 事务信息状态
    # 定义表名
    __tablename__ = 'cyc_affair_status'
    status_type = db.Column(db.Integer, primary_key=True)
    status_explain = db.Column(db.String(3))
